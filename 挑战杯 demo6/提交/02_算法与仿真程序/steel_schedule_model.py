...
from project_paths import create_run_directory, run_result_dir, run_upload_dir
...
def main() -> None:
    ...
    if args.output is None:
        run_dir = create_run_directory(f"cli_{uuid.uuid4().hex[:8]}")
        out = run_result_dir(run_dir)
        upload_dir = run_upload_dir(run_dir)
        shutil.copy2(args.input, upload_dir / Path(args.input).name)
        if args.speed_table and Path(args.speed_table).exists():
