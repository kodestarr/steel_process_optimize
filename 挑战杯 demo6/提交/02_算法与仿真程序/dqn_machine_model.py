...
from project_paths import CODE_ROOT, SUBMISSION_ROOT


def _resolve_model_path(path: str | Path) -> Path:
    p = Path(path)
    candidates = [p] if p.is_absolute() else [CODE_ROOT / p]
    for candidate in candidates:
        resolved = candidate.resolve()
        try:
            resolved.relative_to(SUBMISSION_ROOT)
        except ValueError as exc:
            raise ValueError(f"Model path escapes submission directory: {path}") from exc
        if resolved.exists():
            return resolved
...
class DqnMachineModel:
    ...
    def save(self, path: str | Path) -> None:
        ...
        torch.save(self.net.state_dict(), resolved)
        if self.meta:
