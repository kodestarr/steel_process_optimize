...
def _safe_result_path(run_id: str, filename: str | None = None) -> Path:
    """Resolve a path inside one run workspace, rejecting traversal attempts."""
    ...
    run_root = find_run_directory(safe_id)
    if run_root is None:
        raise HTTPException(404, "运行记录不存在")
    result_root = run_result_dir(run_root)
    if filename is not None:
        ...
        if rel.parts and rel.parts[0] in {ResultStore.STAGING, ResultStore.GENERATIONS, ResultStore.CURRENT}:
            ...
            base = ResultStore(result_root).active_dir()
        resolved = (base / rel).resolve()
    else:
        resolved = run_root.resolve()

    # Ensure the resolved path remains inside the run directory.
    try:
...
# os.sep is used by the SPA static-file path guard below.
import os as _os
...
def _save_history(history: dict) -> None:
    with _HISTORY_LOCK:
        for record in history.get("runs", []):
            run_id = str(record.get("id") or "")
            run_dir = find_run_directory(run_id) if run_id else None
            if run_dir is None:
                continue
            safe = _sanitize_json(record)
            run_history_file(run_dir).write_text(
...
async def upload_excel(file: UploadFile = File(...), speed_file: UploadFile = File(None)):
    ...
    run_dir = create_run_directory(file_id)
    upload_dir = run_upload_dir(run_dir)
    ...
    _atomic_history_update(lambda history: {
        "runs": [
            *[
            ...
            {
                "id": file_id,
                "name": f"运行 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
...
async def run_model(payload: dict):
    ...
    run_dir = find_run_directory(file_id)
    if run_dir is None:
        raise HTTPException(404, "运行记录不存在，请重新上传数据")
    upload_dir = run_upload_dir(run_dir)
    ...
    # 每个运行目录独立保存结果，目录名包含运行时间和运行ID。
    run_id = file_id
    result_store = ResultStore(run_result_dir(run_dir))
    ...
    _atomic_history_update(lambda history: {
        "runs": [
            *[item for item in history.get("runs", []) if item.get("id") != run_id],
            record,
        ]
    })

    _generate_run_reports(run_dir, run_id)
...
def _report_path(run_dir: Path, mode: str) -> Path:
    filename = (
        "建模报告_产能优先.docx"
        if mode == "capacity"
        else "建模报告_兼顾三指标.docx"
    )
    return Path(run_dir) / filename


def _generate_single_report(run_dir: Path, run_id: str, mode: str) -> Path:
    store = ResultStore(run_result_dir(run_dir))
    result_dir = store.active_dir()
    if mode == "balanced":
        result_dir = result_dir / "balanced"
    if not result_dir.exists():
        raise HTTPException(404, "结果目录不存在，无法生成报告")

    build_script = PROJECT_ROOT / "build_report.py"
    if not build_script.exists():
        raise HTTPException(500, "build_report.py 未找到")

    output = _report_path(run_dir, mode)
    try:
        subprocess.run(
            [
                sys.executable,
        ...
        raise HTTPException(500, f"报告生成失败: {exc.stderr}") from exc
    except subprocess.TimeoutExpired as exc:
        raise HTTPException(500, "报告生成超时") from exc
    return output


def _generate_run_reports(run_dir: Path, run_id: str) -> None:
    for mode in ("capacity", "balanced"):
        try:
            _generate_single_report(run_dir, run_id, mode)
        except HTTPException as exc:
            print(f"[REPORT] {mode} 生成失败: {exc.detail}")
...
async def download_report(run_id: str, mode: str = "capacity"):
    ...
    run_root = _safe_result_path(run_id)
    if not run_root.exists():
        raise HTTPException(404, "运行记录不存在")
    if mode not in {"capacity", "balanced"}:
        raise HTTPException(400, "报告模式必须为 capacity 或 balanced")

    docx_path = _report_path(run_root, mode)
    if not docx_path.exists():
        _generate_single_report(run_root, run_id, mode)
...
async def get_history():
    ...
    valid = [r for r in runs if find_run_directory(str(r.get("id", ""))) is not None]
...
async def get_run_detail(run_id: str):
    ...
    run_root = find_run_directory(run_id)
    if run_root is None or not run_root.exists():
        raise HTTPException(404, "运行记录不存在")
    store = ResultStore(run_result_dir(run_root))
...
def _run_dynamic_job(job_id: str, run_id: str, payload: dict, cancel_event) -> None:
    ...
    try:
        ...
        run_dir = find_run_directory(run_id)
        if run_dir is not None:
            ResultStore(run_result_dir(run_dir)).discard_staging(job_id)
        with _dynamic_job_lock:
            _dynamic_jobs[job_id] = {
...
async def dynamic_reschedule(payload: dict):
    ...
    try:
        ...
        result = _sanitize_json(service.reschedule(payload))
        run_dir = find_run_directory(str(run_id))
        if run_dir is not None:
            _generate_run_reports(run_dir, str(run_id))
        return result
    except HTTPException:
        raise
...
async def reschedule_legacy(payload: dict):
    ...
    if not run_root.exists():
        raise HTTPException(404, "运行记录不存在")
    run_dir = ResultStore(run_result_dir(run_root)).active_dir()
    ...
    upload_dir = run_upload_dir(run_root)
...
FRONTEND_DIST = FRONTEND_BUILD_DIR
