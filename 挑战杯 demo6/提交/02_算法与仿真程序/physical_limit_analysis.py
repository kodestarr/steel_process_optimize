...
from project_paths import create_run_directory, run_result_dir, run_upload_dir  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--speed-table", default=None, type=Path)
    parser.add_argument("--output", default=None, type=Path)
    args = parser.parse_args()

    if args.input is None or args.speed_table is None:
        run_dir = create_run_directory(f"physical_{uuid.uuid4().hex[:8]}")
        upload_dir = run_upload_dir(run_dir)
        if args.input is None:
            args.input = INPUT_DIR / "附件2：钢板零件数据.xlsx"
        if args.speed_table is None:
            args.speed_table = INPUT_DIR / "附件3：工艺用时计算表.xlsx"
        shutil.copy2(args.input, upload_dir / Path(args.input).name)
        shutil.copy2(args.speed_table, upload_dir / Path(args.speed_table).name)
    else:
        run_dir = None
    out = args.output or run_result_dir(run_dir)
    ...
    findings_text = "\n".join(findings)
    (out / "findings.txt").write_text(findings_text + "\n", encoding="utf-8")
