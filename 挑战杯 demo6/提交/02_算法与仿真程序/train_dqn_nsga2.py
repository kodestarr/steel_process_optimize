...
The default source is `03_赛题与输入数据`, containing the official 附件2 and
附件3 files. If your files are elsewhere, pass them explicitly:
...
from project_paths import INPUT_DIR, create_run_directory, run_result_dir
...
def main() -> None:
    ...
    if args.output is None:
        run_dir = create_run_directory(f"train_dqn_{uuid.uuid4().hex[:8]}")
        args.output = run_result_dir(run_dir) / "dqn_machine_model.pt"
    ...
    if args.plate_file is not None:
        ...
        if direct_plate.exists():
            datasets = [("正式输入数据", direct_plate, direct_speed)]
        else:
            datasets = []
