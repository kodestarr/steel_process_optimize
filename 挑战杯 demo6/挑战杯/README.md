...
| ⚡ 动态响应 | 多资源、多故障链式事件；切割头/胎架/配套打磨机/AGV 等独立停用；订单取消/改优先级；在制钢板继续加工或中断清台 |
...
动态页面使用状态化重排，不要求每次都从最初排程重算：

- `状态快照`：判断钢板已完成、加工中或未开始，读取资源/缓存/在制状态；
- `贪心修复`：先构造一版满足故障资源和能力依赖的快速可行解；
- `稳定修复`：纯设备故障保持原顺序、原切割机和原胎架，只向后平移受影响任务，避免故障后完工时间反而缩短；
- `策略选择`：系统自动在稳定修复、局部重优化和全局重优化之间选择；
- `订单换数据`：订单变更时必须上传全新的附件2，重新执行数据校验和零件关联，支持保留在制状态、取消未投料钢板和修改未投料优先级；
- `全链路仿真`：重排后重新计算打磨、坡口、AGV、缓存和齐套 KPI；
- `链式事件`：多故障可重叠，支持提前修复、故障开始时间调整和下一次继续重排。

动态排程结果会写入对应运行目录的 `dynamic_state.json`、`dynamic_schedule.csv` 和 `dynamic_stages.csv`。
