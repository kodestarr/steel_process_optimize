...
def pretrain(
    ...
    if output_path is None:
        run_dir = create_run_directory(f"train_drl_{uuid.uuid4().hex[:8]}")
        output_path = run_result_dir(run_dir) / "drl_qtable.npy"
    ...
    for ds_idx, ds_path in enumerate(datasets):
        ...
        for start_name, init_order in init_solutions.items():
            ...
            for it in range(iterations_per_dataset // len(init_solutions)):
                ...
                new_load_diff = met.get("切割负载差(h)", current_met.get("切割负载差(h)", 0.5))
