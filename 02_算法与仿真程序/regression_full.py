...
input_dir = INPUT_DIR
datasets = [(input_dir.name, input_dir / '附件2：钢板零件数据.xlsx', input_dir / '附件3：工艺用时计算表.xlsx')]
results = []
...
for r in results:
    if r['dataset'] == input_dir.name and not r['error']:
        print(f"\n=== Competition KPI Validation ({input_dir.name}) ===")
