...
from project_paths import EVIDENCE_DIR, create_run_directory, ensure_output_dirs
...
def section_algorithm(doc: Document, summary: dict) -> None:
    ...
    add_table(
        ...
        [
            [
                "构造式规则",
                "只要求快速得到一版可行基线，或现场设备/订单正在变化，需要先保证可执行",
                "计算量最低、结果稳定、逻辑容易解释",
                "优化空间有限，通常作为基线和应急方案",
            ],
            [
                "SA+Tabu",
                "钢板数量中等、计算时间有限、顺序调整对交期或齐套影响较大",
                "邻域搜索快，降温与 Tabu 能兼顾探索和避免循环",
                "需要合理设置迭代次数和初温，单次只直接输出一个主要解",
            ],
            [
                "GA+LNS",
                "分段数量多、同分段钢板需要成簇，且希望保持种群多样性",
                "交叉与破坏—修复能跨区域搜索，适合中等规模批排",
                "评估次数较多，响应时间通常高于 SA+Tabu",
            ],
            [
                "DQN+NSGA-II",
                "产能、齐套、负载和等待需要同时呈现，且 N2/N5 机器分配本身也是决策对象",
                "一次输出 Pareto 前沿，同一前沿可选产能优先和综合交付解",
                "模型与训练数据分布有关，搜索时间最长",
            ],
            [
                "DRL-guided SA",
                "数据规模有限，但不同搜索阶段需要自动切换邻域和权重",
                "根据搜索进度、温度和趋势选择算子，自适应性强",
                "效果依赖 Q 表训练质量，动态权重不如 Pareto 前沿直观",
            ],
            [
                "动态修复算法",
                "设备故障、撤单、插单或订单时间变化，要求快速恢复正式可行排程",
                "稳定右移、局部或全局重排可选择，适合在线响应",
                "必须先建立在制状态和故障日历，重大变化仍需重新仿真",
            ],
        ],
        [1.15, 2.05, 1.80, 1.65],
        font_size=7.9,
...
def section_environment(doc: Document, summary: dict) -> None:
    ...
    add_para(
        doc,
        "Web 运行方式：双击 start.bat 或运行 start.py 启动服务，在数据导入页上传附件 2，"
...
def section_engineering(doc: Document, summary: dict) -> None:
    ...
    add_para(
        doc,
        "Web 端通过 start.bat 或 start.py 启动。用户上传附件 2，可选上传附件 3，"
...
def appendix(doc: Document, summary: dict) -> None:
    ...
    add_table(
        ...
        [
            ...
            ["05_系统输出/算法结果/optimized_plate_schedule.csv", "标准钢板排程输出"],
            ["05_系统输出/算法结果/part_completion.csv", "标准零件完成输出"],
            ["05_系统输出/算法结果/process_stages.csv", "标准工序事件输出"],
        ],
        [3.10, 3.50],
        font_size=8.7,
    )
...
def build() -> None:
    ensure_output_dirs()
    run_dir = create_run_directory(f"report_{uuid.uuid4().hex[:8]}")
    output = run_dir / "船体加工车间智能排产与齐套配盘优化调度技术方案报告.docx"
