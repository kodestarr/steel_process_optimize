...
求解层并列提供SA+Tabu、GA+LNS、DQN可选种子增强的NSGA-II和DRL-guided SA等算法接口。正式主实验采用NSGA-II联合优化，在同一Pareto前沿上分别选择产能优先解和兼顾产能、齐套、负载的平衡解；SA+Tabu、GA+LNS和DRL路径保留为对照与快速响应方案。本提交包未附经训练的DQN权重，程序按设计自动采用多样化启发式机器码初始化，因而本次结果不作为DQN增益的单独证明。各算法调用同一仿真评价器，以总完工时间、加权平均齐套跨度、切割负载差、等待时间和设备利用率为核心指标。当前数据包含\PlateCount 张钢板、\PartCount 个零件和\KitGroupCount 个分段优先级齐套组。正式复现实验数值由\code{report\_data.tex}统一维护，避免正文、图表和程序输出出现多套口径。
...
固定随机种子20260723、种群规模16、有效代数10的NSGA-II多目标实验下，综合交付解将总完工时间由FIFO的65.86小时降至60.09小时，加权平均齐套跨度由15.07小时降至11.79小时，整体产能提高至14.51张/班；产能优先备选解进一步将总完工时间降至58.69小时。两条轨道均未出现死锁警告。坡口工作站负荷与暂定容量之比仍高于100\%，报告将其列为现场参数校准风险。

\noindent\textbf{关键词：}船体加工；智能排产；齐套配盘；离散事件仿真；NSGA-II；SA+Tabu；GA+LNS；多目标优化；动态重调度；紧急插单
...
NSGA-II是当前正式主算法，适用于钢板顺序和N2/N5机器分配需要联合优化、且计划人员希望一次看到多种目标取舍的场景。系统同时预留DQN机器码种子接口：DQN模块按16维状态输入，经$16\rightarrow128\rightarrow64\rightarrow5$网络输出固定动作维，正式场景中仅使用N2/N5对应的合法动作。状态包含板厚、板宽、切割工时、齐套优先级、机器负载和剩余任务量。

DQN只用于生成少量候选机器码，不替代完整排产仿真。当PyTorch、权重文件、特征范围检查不通过或训练数据分布不匹配时，系统自动回退到负载均衡、N2偏好、N5偏好和随机扰动组成的启发式机器码。本提交包未附经训练的DQN权重文件及其元数据，因此本次固定种子实验实际采用该启发式回退路径；报告中的改进结果应归因于NSGA-II联合搜索与统一仿真评价，不宣称已经验证DQN带来的独立提升。
...
随机种子 & 20260723 \\
主验证算法 & NSGA-II联合优化，启发式机器码种子，种群16，有效代数10，单进程复现 \\
...
以下静态页面截图取自同一组正式运行，保留了当前输入、算法轨道、运行记录和主要指标；动态重调度截图以该排程为基础注入N2故障并叠加订单变更。它们依次证明数据与参数可配置、排程结果可解释、资源与齐套状态可追踪，以及异常发生后能够在线重排。
...
本提交包固定附件2、附件3、随机种子20260723、NSGA-II种群规模16、有效代数10、启发式机器码种子和单进程评估配置，并保存产能优先轨道与综合交付轨道的\code{summary.json}、CSV与PNG。目录名及程序界面沿用“DQN+NSGA-II”算法族名称，但由于提交包未附训练权重，本次可复现路径为启发式种子加NSGA-II。所有正文指标均由这两份汇总文件生成。时间优先模式开启时，系统把最大计算时间的约95\%作为优化预算；关闭时完全按模型参数运行。复现时应保持上述配置一致；如修改模型约束或参数，应把新结果保存到独立目录并同步更新报告数据，避免跨版本拼接指标。
...
算法交付 & NSGA-II联合优化为正式复现主路径，DQN为具备权重时启用的可选种子接口；SA+Tabu、GA+LNS和DRL作为对照或快速响应路径 & 统一仿真评价接口、Pareto前沿、回退机制和源代码 \\
